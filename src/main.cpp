...
int main()
{
    ...
    auto ui_camera = bifrost::GenUICamera(screen_size.x, screen_size.y);

    glfwSetWindowUserPointer(window, &ui_camera);
    glfwSetFramebufferSizeCallback(window, [](GLFWwindow* window, int width, int height)
    {
        glViewport(0, 0, width, height);
        auto ui_camera = (bifrost::Camera2d*)glfwGetWindowUserPointer(window);
